...
import com.mitidimitara.exchangeRate.domain.util.commasToDots
...
class ConversionViewModel @Inject constructor(
    ...
    private val amountForConversion: Double = (savedStateHandle.get<String>(ConversionState.KEY_AMOUNT)?.commasToDots()?.toDouble()) ?: 0.0
    ...
    fun recalculateConversion(amount: String) {
        if (amount.isNotEmpty()) {
            val conversionRateNormal = (toCurrency.second / fromCurrency.second) * amount.commasToDots().toDouble()
            val conversionRateReversed = (fromCurrency.second / toCurrency.second) * amount.commasToDots().toDouble()

            _uiState.value = _uiState.value.copy(
                amountForConversion = amount.commasToDots(),
                conversionRateNormal = conversionRateNormal.roundTwoDecimals(),
                conversionRateReversed = conversionRateReversed.roundTwoDecimals()
            )
        } else {
            _uiState.value = _uiState.value.copy(
                amountForConversion = (0.0).toString(),
                conversionRateNormal = (0.0).toString(),
                conversionRateReversed = (0.0).toString(),
            )
        }
    }
}